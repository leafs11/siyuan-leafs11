    ...
    build: {
      ...
      rollupOptions: {
        plugins: [
          ...(isWatch
            ? [
                // 注意：已移除 livereload 插件。
                // 它在思源 https 伺服下会注入 http://host:35729/livereload.js，
                // 导致控制台报 ERR_SSL_PROTOCOL_ERROR，且对插件开发无实际作用。
                {
                  // 监听静态资源文件
                  name: "watch-external",
                  buildStart: async function(this: any) {
